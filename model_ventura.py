...
ventura.drop(columns=['Orange', 'Ventura'], inplace=True)

# split the data into 3 data sets and 3 target arrays
X_train, X_validate, X_test, y_train, y_validate, y_test = wr.full_split_zillow(ventura)
...
f5 = ['bedrooms','bathrooms','sq_feet','house_age','pools']
...
def run_all_models():
    ...
    run_polynomial()
    run_single()
    scores.to_csv('regression_results_ventura.csv')
